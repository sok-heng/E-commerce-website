...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$current_password || !$new_password) {
        header("Location: setting.php?error=" . urlencode("Please fill in both password fields."));
        exit;
    }
    ...
    if (!$hashed_password) {
        header("Location: setting.php?error=" . urlencode("User not found."));
        exit;
    }

    if (!password_verify($current_password, $hashed_password)) {
        header("Location: setting.php?error=" . urlencode("Current password is incorrect."));
        exit;
    }
    ...
    if ($update_stmt->execute()) {
        $update_stmt->close();
        header("Location: setting.php?success=" . urlencode("Password updated successfully."));
        exit;
    } else {
        $update_stmt->close();
        header("Location: setting.php?error=" . urlencode("Failed to update password."));
        exit;
    }
}
?>
...
<a href="/FN/e-commerce/homepage.php"
  ...
  <div class="grid grid-cols-8 pt-10 sm:grid-cols-10">
    ...
    <div class="col-span-8 overflow-hidden rounded-xl  sm:px-8 sm:shadow">
      <div class="pt-4">
        <h1 class="py-2 text-2xl font-semibold"><span class="text-pink-400">Account</span> settings</h1>
      </div>
      <hr class="mt-4 mb-8" />
      ...
      <form method="POST" action="setting.php">
        ...
        <button
          type="submit"
          class="mt-4 rounded-lg mb-5 bg-pink-400 px-4 py-2 text-white hover:bg-pink-700"
        >
          Save Password
        </button>
      </form>
