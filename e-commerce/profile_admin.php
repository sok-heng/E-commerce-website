...
if (isset($_POST['save_profile'])) {
    // Fetch user role first to verify admin before allowing update
    ...
    if (!$userRoleRow || $userRoleRow['user_role'] !== 'admin') {
        die("❌ Access denied: Only admins can update this profile.");
    }
    ...
    // Case 1: Upload image file from PC
    if (isset($_FILES['user_profile']) && $_FILES['user_profile']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedExts)) {
            ...
            $_SESSION['message'] = "❌ Unsupported file extension for upload.";
            header("Location: profile_admin.php");
            exit;
        }
    }
    // Case 2: Paste image URL (only if no uploaded file)
    elseif (!empty($_POST['user_profile_url'])) {
        $imageUrl = trim($_POST['user_profile_url']);
        $imgExt = pathinfo(parse_url($imageUrl, PHP_URL_PATH), PATHINFO_EXTENSION);
        $validExts = ['jpg', 'jpeg', 'png', 'gif', 'webp'];

        if (in_array(strtolower($imgExt), $validExts)) {
    ...
    <li class="cursor-pointer px-3 py-2 text-sm text-slate-600 hover:bg-blue-700 hover:text-white"><a href="./message.php">Messages</a></li>
       ...
       <ul>
        <li class="mt-5 cursor-pointer border-l-2 border-transparent px-2 py-2 font-semibold transition hover:border-l-blue-700 hover:text-blue-700"><a href="./dashboard.php">Dash Board</a></li>
         ...
         <a href="setting_admin.php">Accounts</a> 
        </li>
      ...
      <li class="mt-5 cursor-pointer border-l-2 border-l-blue-700 px-2 py-2 font-semibold text-blue-700 transition hover:border-l-blue-700 hover:text-blue-700"><a href="./profile_admin.php">Profile</a></li>
        ...
        <div class="col-span-8 overflow-hidden rounded-xl  sm:px-8 sm:shadow mb-8">
            ...
            <form class="space-y-4" method="POST" enctype="multipart/form-data" action="profile_admin.php">
                <div class="flex flex-col md:flex-row justify-between mb-5 items-start pt-8">
                    <h2 class="mb-5 text-4xl font-bold"><span class="text-pink-400">Update</span> Profile</h2>
                    <div class="text-center">
                        <img src="<?= htmlspecialchars($imgPath ?? '') ?>"
                             alt="Profile Picture"
                             class="rounded-full w-32 h-32 mx-auto border-4 border-pink-400 mb-4 transition-transform duration-300 hover:scale-105 ring ring-gray-300">
                ...
                <div class="flex justify-end space-x-4 pb-5">
                    <button type="reset" class="px-4 py-2 bg-pink-400 text-white  rounded-lg hover:bg-white hover:text-pink-400 hover:border hover:border-pink-400">Cancel</button>
                    <button type="submit" name="save_profile"
                            class="px-4 py-2 border border-pink-400 text-pink-400 rounded-lg hover:bg-pink-400 hover:text-white ">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
